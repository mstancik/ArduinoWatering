...
// 2021-08-29 akutne prerobenie programu na pevne casy napustania po pokazeni senzoru % nadoby
...
int lZostavajuciCasNaplnania;   // aktualny cas v sekundach kolko sa bude naplnat nadoba
...
void loop() {
  ...
  while (1) {
    ...
    if (oldSecond != second()) { // raz za sekundu prepis display
      ...
      if (fCerpadlo && lZostavajuciCasNaplnania > 0)
...
bool jeVodaNaMax() {
  ...
  if (sensorValueMax < VODA_PRAH + 100)
    return 1;
  else
    return 0;
}
...
void naplnajNadobu() {
  ...
  if (senzorStudna){
    ...
    if (lZostavajuciCasNaplnania <= 0 || !jeVodaNaMin()) { // nadoba plna alebo v studni min elektroda neni zaliata vodou
      digitalWrite(11, HIGH); // vypni cerpadlo studne
      fCerpadlo = false;
  ...
  if (sekundOdVypnutiaStudne == 0 && !fCerpadlo){
    ...
    if (jeVodaNaMax() && jeVodaNaMin() && lZostavajuciCasNaplnania > 0) {
